...
Практика 1. Разработка безопасного программного обеспечения
...
Задание должно быть лично защищено преподавателю до 1 декабря 2025 и загружено в online-edu.mirea.ru

группы: 

Ссылка на курс: https://sdlc.yonote.ru/share/SDLC_Lab_1 
...
Обход путей (Path Traversal): Пользователям нельзя давать возможность обращаться к файлам вне предопределенного каталога.

Небезопасная десериализация (Insecure Deserialization): Необходимо защитить приложение от выполнения нежелательного кода при обработке JSON/XML.

Атаки через ZIP-бомбы (ZIP Bomb): Нужно ограничить максимальную степень сжатия архива, чтобы исключить риск отказа в обслуживании.

Race Conditions: Следует обеспечить атомарный доступ к важным ресурсам, исключив конфликты при параллельном доступе.

Хранимые процедуры и подготовка запросов (Prepared Statements): Все запросы к базе данных должны исполняться через подготовленные инструкции, предотвращая возможные инъекции.

Дополнительные меры безопасности

Установка максимальных размеров файлов для загрузки и сохранения.

Использование безопасного способа копирования и перемещения файлов.

Блокировка несанкционированного просмотра файлов и папок.

Логирование действий пользователей:

Вся деятельность пользователей фиксируется в журнале активности, хранимом в базе данных.

SQL Injection: Данные, поступающие от пользователя, должны фильтроваться и экранироваться перед отправкой SQL-запросов.
...
Получение списка дисков и свойств файловой системы:
...
Управление файлами в форматах JSON и XML:
...
Использование хранимых процедур и подготовленных инструкций:
...
Предотвращение условий состязания (Race Conditions):
...
Understanding the “Zip of Death” and How to Defend Against It

Небезопасная многопоточность или Race Condition

Exploiting and preventing insecure deserialization vulnerabilities

https://www.geeksforgeeks.org/php/path-traversal-attack-prevention/
